...
def iter_mri_dataset_with_tabular_variable(dataset_root, side="left", max_patients=None, tabular_variable="V00KOOSKPL", clinical_csv_path="./csv/clinical00_cleaned.csv"):
    """
    Generator that yields (patient_id, tensor[1, D, 224, 224], tabular_variable)
    ...
    all_ids: List[str] = list_patient_ids_fast(dataset_root, side, max_patients)
    ...
    clinical_dict = df.set_index("ID").to_dict("index")
    ...
    for subset in subset_dirs:
        ...
        for patient_id in os.listdir(subset_path):
            ...
            if tensor is not None:
                ...
                yield patient_id, tensor, torch.tensor([clinical_dict[patient_id][tabular_variable]])
