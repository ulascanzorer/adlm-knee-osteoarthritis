...
# -------------------------------------------------
# Tabular encoder (mask-aware)
# -------------------------------------------------

class TabularEncoder(nn.Module):
    """
    Encodes tabular values + missingness mask.
    ...
    def __init__(
        ...
        self.encoder = nn.Sequential(
            nn.Linear(num_tabular_inputs * 2, hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, tab_x: torch.Tensor, tab_mask: torch.Tensor) -> torch.Tensor:
        if tab_x.dim() == 1:
            tab_x = tab_x.unsqueeze(0)
        if tab_mask.dim() == 1:
            tab_mask = tab_mask.unsqueeze(0)

        x = torch.cat([tab_x, tab_mask], dim=1)
        return self.encoder(x)


# -------------------------------------------------
# Autoencoder with tabular input
# -------------------------------------------------
...
class AutoencoderWithTabularInput(nn.Module):
    ...
    def forward(
        ...
        encoded_tab = self.tabular_encoder(tab_x, tab_mask)   # (B,T_lat)
